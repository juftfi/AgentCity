#!/usr/bin/env node
/**
 * Agent City MCP Server — stdio-based MCP server for auggie.
 *
 * Exposes a `dispatch_to_worker` tool that lets the LLM delegate tasks to
 * specific workers in Agent City.  Reads the worker roster from the
 * AGENT_TOWN_WORKERS env var (JSON array) and dispatches via HTTP POST to
 * the internal dispatch endpoint on the Agent City server.
 *
        ...
        tools: [
          {
            ...
            description: `Dispatch a task to another worker in Agent City. The worker will execute the task independently and return the result. Use this to delegate work to team members based on their specialty.\n\nAvailable workers:\n${workerListDescription()}`,
            inputSchema: {
              type: "object",
              properties: {
                seatId: {
